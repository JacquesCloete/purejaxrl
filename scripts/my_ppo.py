# Jacques' re-implementation of (continuous action) PPO in JAX, following purejaxrl/ppo_continuous_action.py.
...
class ActorCritic(nn.Module):
    ...
    def __call__(self, x):
        ...
        actor_logstd = self.param("logstd", nn.initializers.zeros, (self.action_dim,))
...
def make_train(config):
    ...
    config["MINI_BATCH_SIZE"] = (config["NUM_ENVS"] * config["NUM_STEPS"]) // config[
        "NUM_MINIBATCHES"
    ]
            ...
            def _update_epoch(update_state, _):
                def _update_minibatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, traj_batch, gae, targets):
                        ...
                        actor_loss = -jnp.minimum(actor1_loss, actor2_loss).mean()
                ...
                rng, _rng = jax.random.split(rng)
                batch_size = config["MINI_BATCH_SIZE"] * config["NUM_MINIBATCHES"]
                assert (
